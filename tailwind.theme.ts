	...
	Object.entries(input).forEach(([key, value]) => {
		...
		if (typeof value !== 'string' && !Array.isArray(value)) {
			...
			if (newValue.startsWith('--')) {
				output[`--${key === 'DEFAULT' ? path.join('-') : newPath.join('-')}`] = 'var(' + newValue + ')'
			} else {
				output[`--${key === 'DEFAULT' ? path.join('-') : newPath.join('-')}`] = getRgbChannels(newValue)
			}
		}
	})
	return output
	...
	return Object.fromEntries(
		Object.entries(input).map(([key, value]) => {
			...
			if (typeof value !== 'string' && !Array.isArray(value)) {
				...
				return [key, `rgb(var(--${key === 'DEFAULT' ? path.join('-') : newPath.join('-')}) / <alpha-value>)`]
			}
		})
	)
}
...
export const themePreset = {
	darkMode: 'selector',
	content: [],
	plugins: [themePlugin, formPlugin],
	theme: {
		extend: {
